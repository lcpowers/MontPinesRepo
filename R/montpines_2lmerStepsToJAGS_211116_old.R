...
# ## LOAD DATA --------------------------------------------------------------------------------------
# # APRIL:
# er_dats <- read.csv("../April's eriogonum files/erbr_TagClust_210510.csv", header = TRUE)
# 
# # MONT PINES  
# mp_dats <- read.csv("../fullannual data A.csv", header = T) %>% select(-1) %>% 
#   filter(TagNo!=0 & TagNo != "1(no tag)") %>% # Removing first column -- looks like a left over row number column for prior saving. 
...
# er_Nallrows <- length(er_dats$Site) # 2208
# mp_Nallrows <- length(mp_dats$Site) # 15584
# 
# ####### number of years -- ##### What's the difference between demoyr and yr?
# 
# # Years in data
# er_numyears <- length(unique(er_dats$Year))
# mp_numyears <- length(unique(mp_dats$yr))
# 
# ###### Do we have transects? ######
# er_numtrans <- length(unique(er_dats$TransectNew))
# 
# ## Identify rows that are good dependent values (ending sizes) for surv or growth  
# # Lags 
# er_lagforsurv <- er_dats$lagforsurv #Full list of lags or of -1 for first observation rows
# mp_lagsforsurv <- mp_dats$lags
# 
# # ER good rows(years with data)
# er_goodrows <- which(er_dats$lagforsurv > 0)
# er_goodgrowrows <- which(er_dats$lagsrtsz > 0)
# 
# # MP good rows --
# mp_goodrows <- which(mp_dats$lags>0)
# mp_goodrows <- NULL ##### Do we column have goodgrowrows? #####

# ## Identify the lag for these rows: how far back is the last good size measurement?
# mp_lagvals <- mp_dats$lags[mp_goodrows]
# mp_Ncases <- length(goodrows)
# # Ngrowcases <- length(goodgrowrows)
# Survs <- mp_dats$surv



# From April's code -- not applicable to MPs
# RosNew <- dats$RosNew
