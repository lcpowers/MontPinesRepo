...
for(i in 1:nrow(montpines)){
  ...
  montpines$TempMaySept1[i] = ifelse((yr-1)%in%unique(montpines$yr[montpines$Site==site]),
                                     climate$Temp.May.Sept.[climate$Site==site&climate$yr==(yr-1)],NA)
...
jags.mod <- run.jags('R/montpines_JAGSmodel_EMW.R', # Call to specific jags model
                     n.chains=3,
                     data=mp4jags,
                     burnin=50,
                     thin=10,
