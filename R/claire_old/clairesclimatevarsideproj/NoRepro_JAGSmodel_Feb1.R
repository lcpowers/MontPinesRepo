...
model{
  ...
  grwth_TempMaySeptCoef ~ dunif(tmayseptmin,tmayseptmax)
  grwth_TempOctAprCoef ~ dunif(toctaprmin,toctaprmax)
  grwth_PrecipAugJulyCoef ~ dunif(precipmin,precipmax)
  grwth_fogCoef ~ dunif(fogmin,fogmax)
  ...
  surv_TempMaySeptCoef ~ dnorm(tmayseptmin,tmayseptmax)
  surv_TempOctAprCoef ~ dnorm(toctaprmin,toctaprmax)
  surv_PrecipAugJulyCoef ~ dnorm(precipmin,precipmax) 
  surv_fogCoef ~ dnorm(fogmin, foxmax)
  surv_cloudCoef ~ dnorm(cldmin,cldmax)
...
#data# Ncases,Ngrowcases,goodrows,goodgrowrows,lagvals,DBH,TempMaySept,TempOctApr,PrecipAugJuly,fog,cloud,transect.num,cldmin,cldmax,fogmin,fogmax,tmayseptmin,tmayseptmax,toctaprmin,toctaprmax,precipmin,precipmax
...
#   "grwth_intercept" <- 2
#   "grwth_dbhCoef" <- 1
#   "grwth_TempMaySeptCoef" <- TempMaySept_init
#   "grwth_TempOctAprCoef" <- TempOctApr_init
#   "grwth_PrecipAugJulyCoef" <- PrecipAugJuly_init
#   "grwth_fogCoef" <- Fog_init
#   "grwth_cloudCoef" <- Cloud_init
...
#   "surv_intercept" <- -1
#   "surv_dbhCoef" <- 0.01
#   "surv_TempMaySeptCoef" <- TempMaySept_init
#   "surv_TempOctAprCoef" <- TempOctApr_init
#   "surv_PrecipAugJulyCoef" <- PrecipAugJuly_init
#   "surv_fogCoef" <- Fog_init
#   "surv_cloudCoef" <- Cloud_init
...
inits{
  ...
  "grwth_intercept" <- 1.5
  "grwth_dbhCoef" <- 1
  "grwth_TempMaySeptCoef" <- TempMaySept_init[1]
  "grwth_TempOctAprCoef" <- TempOctApr_init[1]
  "grwth_PrecipAugJulyCoef" <- PrecipAugJuly_init[1]
  "grwth_fogCoef" <- Fog_init[1]
  "grwth_cloudCoef" <- Cloud_init[1]
  "grwth_Transect_precision" <- 2
  ...
  "surv_intercept" <- -10
  "surv_dbhCoef" <- -0.01
  "surv_TempMaySeptCoef" <-TempMaySept_init[1]
  "surv_TempOctAprCoef" <- TempOctApr_init[1]
  "surv_PrecipAugJulyCoef" <- PrecipAugJuly_init[1]
  "surv_fogCoef" <- Fog_init[1]
  "surv_cloudCoef" <- Cloud_init[1]
  "surv_Transect_precision" <- 0.001
...
inits{
  ...
  "grwth_intercept" <- 1.5
  "grwth_dbhCoef" <- 1
  "grwth_TempMaySeptCoef" <- TempMaySept_init[2]
  "grwth_TempOctAprCoef" <- TempOctApr_init[2]
  "grwth_PrecipAugJulyCoef" <- PrecipAugJuly_init[2]
  "grwth_fogCoef" <- Fog_init[2]
  "grwth_cloudCoef" <- Cloud_init[2]
  "grwth_Transect_precision" <- 2
  ...
  "surv_intercept" <- -20
  "surv_dbhCoef" <- 0.01
  "surv_TempMaySeptCoef" <-TempMaySept_init[2]
  "surv_TempOctAprCoef" <- TempOctApr_init[2]
  "surv_PrecipAugJulyCoef" <- PrecipAugJuly_init[2]
  "surv_fogCoef" <- Fog_init[2]
  "surv_cloudCoef" <- Cloud_init[2]
  "surv_Transect_precision" <- 0.001
